...
6. [Install Helm](https://helm.sh/docs/intro/quickstart/) with at least version 3
...
### Steps
## Deploy Kafka with external access enabled (external access URL: localhost:30005)
 ...
 ### Verifying it works 
    execute 
    1. kubectl get pods 
    You should see the following pods created 
    ...
    from kafka import KafkaProducer
      ...
      producer.send(TOPIC_NAME, b'It is working!!!')
      producer.flush()
...
6. `kubectl apply -f deployment/connection-api.yaml` - Set up the service and deployment for the service that retrieve the connections using person id 
7. `kubectl apply -f deployment/person-api.yaml` - Set up the service and deployment for the persons app API
8. `kubectl apply -f deployment/location-consumer.yaml` - Set up the service and deployment for the Kafka consumer that reads from kafka and insert in database 
